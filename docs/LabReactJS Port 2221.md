...
### 2.2. Bước 1: Copy source code vào WSL

Trong bài lab này, **WSL đóng vai trò máy chủ** (thay cho VPS thực tế). Toàn bộ quá trình build và chạy đều thực hiện trên WSL.

Mở **PowerShell trên Windows**, di chuyển vào thư mục dự án và copy source lên WSL:

```powershell
scp -r -P 2221 . gianglt@localhost:~/simple-reactjs/
```

**Giải thích:**

- `-P 2221`: cổng SSH của WSL
- `.`: toàn bộ thư mục hiện tại (source code)
- `~/simple-reactjs/`: thư mục đích trong home directory của WSL

> **Kết quả mong đợi:** Các file được copy thành công, không có thông báo lỗi.

---

### 2.3. Bước 2: SSH vào WSL

Trong **PowerShell trên Windows**, chạy:

```powershell
ssh -p 2221 gianglt@localhost
```

Nếu lần đầu kết nối, thêm host key vào known_hosts trước:

```powershell
ssh-keyscan -p 2221 localhost >> ~/.ssh/known_hosts
```

Sau đó SSH lại và nhập mật khẩu khi được nhắc.

> **Kết quả mong đợi:** Đăng nhập thành công vào WSL:
> ```
> gianglt@DELL-5570:~$
> ```
...
> - `Connection refused`: dịch vụ SSH trong WSL chưa chạy (chạy `sudo service ssh start`)
...
### 2.7. Bước 6: Build ứng dụng trên WSL

Trong phiên SSH vào WSL, di chuyển vào thư mục source đã copy ở Bước 1:
...
Khi có thay đổi mới, từ **PowerShell trên Windows** copy source mới lên WSL:

```powershell
scp -r -P 2221 . gianglt@localhost:~/simple-reactjs/
```

Rồi SSH vào WSL và build lại:
